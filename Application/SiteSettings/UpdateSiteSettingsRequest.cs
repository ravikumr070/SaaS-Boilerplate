...
public class UpdateSiteSettingsRequest : IRequest<SiteSettingsModel>
{
    public long Id { get; set; }
    public UpdateSiteSettingsRequest(long id) => Id = id;
...
public class UpdateSiteSettingsRequestValidator : CustomValidator<UpdateSiteSettingsRequest>
{
    public UpdateSiteSettingsRequestValidator(IRepository<SiteSettingsModel> repository, IStringLocalizer<UpdateSiteSettingsRequestValidator> localizer) { }

}
public class UpdateSiteSettingsRequestHandler : IRequestHandler<UpdateSiteSettingsRequest, SiteSettingsModel>
{
    // Add Domain Events automatically by using IRepositoryWithEvents
    private readonly IRepositoryWithEvents<SiteSettingsModel> _repository;
    private readonly IStringLocalizer<UpdateSiteSettingsRequestHandler> _localizer;

    public UpdateSiteSettingsRequestHandler(IRepositoryWithEvents<SiteSettingsModel> repository, IStringLocalizer<UpdateSiteSettingsRequestHandler> localizer) =>
        (_repository, _localizer) = (repository, localizer);
    public async Task<SiteSettingsModel> Handle(UpdateSiteSettingsRequest request, CancellationToken cancellationToken)
    {
        var SiteSettings = await _repository.GetByIdAsync(request.Id, cancellationToken);
        _ = SiteSettings ?? throw new NotFoundException(string.Format(_localizer["SiteSettings notfound"], request.Id));
        SiteSettings.Update(
            request.settingsID,
     request.SiteId,
