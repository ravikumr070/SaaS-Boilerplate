...
internal class AzureAdJwtBearerEvents : JwtBearerEvents
{
    ...
    public override async Task TokenValidated(TokenValidatedContext context)
    {
        ...
        var tenant = issuer == _config["SecuritySettings:AzureAd:RootIssuer"]
            ? await tenantDb.Tenants.FindAsync(MultitenancyConstants.Root.Id)
            : await tenantDb.Tenants.FirstOrDefaultAsync(t => t.Issuer == issuer);
        ...
        // Adding tenant claim.
        identity.AddClaim(new Claim(UserClaims.Tenant, tenant.Id));
